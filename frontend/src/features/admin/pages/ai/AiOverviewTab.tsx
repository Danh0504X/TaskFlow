...
const ALERT_ICON = { STUCK_PROCESSING: Clock, QUOTA_CAPPED: AlertTriangle } as const
...
const AiOverviewTab = () => {
  ...
  // "Draft bỏ phí" = draft đã sinh nhưng không được chấp nhận — suy ra thẳng từ 2 chặng cuối
  // của phễu (Draft - Chấp nhận), không cần backend tính riêng.
  const draftStage = data.funnel[1]?.value ?? 0
  const acceptedStage = data.funnel[2]?.value ?? 0
  const wastedDrafts = Math.max(0, draftStage - acceptedStage)
  const wastedDraftsRate = draftStage > 0 ? wastedDrafts / draftStage : 0

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <AdminStatCard label="Tỉ lệ chấp nhận 7 ngày" value={`${(data.acceptanceRate7d * 100).toFixed(0)}%`} staggerIndex={0} />
        <AdminStatCard label="Tỉ lệ lỗi 7 ngày" value={`${(data.errorRate7d * 100).toFixed(1)}%`} staggerIndex={1} />
        <AdminStatCard label="Lượt sinh hôm nay" value={data.generationsToday} staggerIndex={2} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <SectionCard className="p-5 animate-fade-up" style={{ '--stagger': 3 } as CSSProperties}>
          <h3 className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-4">Phễu chuyển đổi (30 ngày)</h3>
          <FunnelChart stages={data.funnel} wastedDrafts={wastedDrafts} wastedDraftsRate={wastedDraftsRate} />
        </SectionCard>

        <SectionCard className="p-5 animate-fade-up" style={{ '--stagger': 4 } as CSSProperties}>
          <h3 className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-4">Tỉ lệ chấp nhận theo loại (30 ngày)</h3>
      ...
      <SectionCard className="p-5 animate-fade-up" style={{ '--stagger': 5 } as CSSProperties}>
        <h3 className="text-[11px] font-semibold text-muted uppercase tracking-wide mb-4">Token &amp; lỗi 30 ngày</h3>
