...
import { CreditCard, Crown, Calendar, CheckCircle2, RefreshCw } from 'lucide-react'
...
export const PaymentHistoryTab: React.FC<PaymentHistoryTabProps> = ({ onOpenPricing }) => {
  ...
  const fetchHistory = async () => {
    try {
      ...
      if (data) {
        ...
        setCurrentPlanExpiresAt(data.currentPlanExpiresAt || null)
        // Chỉ lọc và hiển thị các đơn đã thanh toán thành công (PAID)
        const paidHistory = (data.history || []).filter((tx) => tx.status === 'PAID')
        setHistory(paidHistory)
        if (user) {
          setUser({
            ...user,
            plan: data.currentPlan || 'FREE',
            currentPlanExpiresAt: data.currentPlanExpiresAt || null,
          })
        }
      }
    } catch (err) {
  ...
  return (
    <div className="space-y-6 text-ink">
      ...
      <div className="p-5 rounded-lg bg-surface border border-hairline flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-lg bg-pastel-yellow text-pastel-yellow-ink flex items-center justify-center shrink-0">
            <Crown size={22} />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-base">Gói tài khoản hiện tại:</h3>
              <Badge color={isPro ? 'amber' : 'slate'}>{isPro ? 'PRO' : 'FREE'}</Badge>
            </div>
        ...
        <Button size="sm" onClick={onOpenPricing} className="shrink-0">
      ...
      <div className="space-y-3">
        ...
        {history.length === 0 ? (
          ...
          <div className="py-12 px-4 text-center border border-dashed border-hairline rounded-lg bg-canvas space-y-3">
                ...
                <tbody className="divide-y divide-hairline">
                  {history.map((tx) => (
                    <tr key={tx.id} className="hover:bg-canvas/50 transition-colors">
                      <td className="p-3 font-mono font-bold text-ink">{tx.paymentCode}</td>
                      ...
                      <td className="p-3">
                        {/* history chỉ chứa giao dịch đã PAID (lọc ở fetchHistory) nên trạng thái luôn là 1 kiểu. */}
                        <Badge color="green">
                          <CheckCircle2 size={12} className="mr-1" />
                          Đã thanh toán{tx.resolutionStatus === 'RESOLVED_BY_ADMIN' && ' (Duyệt thủ công)'}
                        </Badge>
                      </td>
                    </tr>
                  ))}
